...
void hm_cmd_uart_ck(void* param)
{
    ...
    if(pcmd_item->buf_idx == sizeof(hm_cmd_req_t))
    {
        ...
        else
        {
            ...
            HM_UART_PRINT( "cmd transfer push %d \r\n", s_cks); 
...
void hm_cmd_msg_ck(void* param)
{
    ...
    if(pcmd_item->pcmd->output_len != 0)
    {
        hm_uart_send(pcmd_item->cmd_buf, sizeof(hm_cmd_req_t));
    }
